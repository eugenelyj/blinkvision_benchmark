...
def check_submission(sample_path, submission_path):
    ...
    list_content = os.listdir(submission_path)
    # list_content must be ['clean', 'final'] or ['event']
    pattern_list = []
    if 'clean' in list_content and 'final' in list_content:
        pattern_list = ['clean', 'final']
    elif 'event' in list_content:
        pattern_list = ['event']
    else:
        return False, "Submission path must contain either 'clean' and 'final' or 'event'"
    ...
    for scene in os.listdir(sample_path):
        ...
        for pattern in pattern_list:
            submission_scene_path = os.path.join(submission_path, pattern, scene)
