...
def read_depth_file(file_path, relative_path, gt_input):
    """Read the depth file format used in the C++ code."""
    with open(file_path, 'rb') as f:
        # Read header
        tag = np.fromfile(f, dtype=np.int32, count=1)[0]
        if tag != TAG_VALUE:
            raise ValueError(f"Invalid tag {tag} in depth file: {file_path}, expected {TAG_VALUE}")
        ...
        contains_vis = False
        depth_relative_path = relative_path.split('.')[0] + '.bin'
        if depth_relative_path in vis_list or depth_relative_path.replace('outdoor_gt', 'outdoor') in vis_list:
            contains_vis = True
...
def sample_and_save_depth(depth_path, sample_map_path, output_path, rel_path, gt_input):
    """Sample depth data according to the sample map and save it."""
    # Read depth data
    width, height, data = read_depth_file(depth_path, rel_path, gt_input)
    depth_data = data['depth']

    depth_data = depth_data.reshape(height, width)
    sample_map = load_sample_map(sample_map_path, height, width)
    if data['contains_vis']:
        sampled_depth = depth_data.reshape(-1)
    else:
        sampled_depth = depth_data[sample_map].reshape(-1)
    ...
    with open(output_file, 'wb') as f:
        ...
        num_samples = int(len(sampled_depth))
        np.array(num_samples).astype(np.int32).tofile(f)
        
        # Write sampled depth data
        sampled_depth.tofile(f)
...
def process_directory(depth_dir, sample_maps_dir, output_dir, gt_input):
    pattern_list = []
    if not gt_input:
        # should be clean&final or event folder under depth_dir
        list_content = os.listdir(depth_dir)
        if len(list_content) == 2 and 'clean' in list_content and 'final' in list_content:
            ...
            raise ValueError(f"Invalid directory structure: {depth_dir}")
    else:
        pattern_list = ['']
    ...
    for pattern in pattern_list:
        for sample_map_path in tqdm.tqdm(sample_maps_path.rglob('*.bin'), total=total_files):
            ...
            # Construct paths
            depth_path = depth_dir_path / pattern / rel_path.with_suffix('.bin')
            output_file = output_path / pattern / rel_path.with_suffix('.bin')
            
            # Process the file
            sample_and_save_depth(depth_path, sample_map_path, output_file, str(rel_path), gt_input)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Sample depth data using pre-generated sample maps')
    parser.add_argument('depth_dir', help='Directory containing depth files')
    ...
    process_directory(args.depth_dir, args.sample_maps_dir, args.output_dir, args.gt_input) 
