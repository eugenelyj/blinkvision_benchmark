...
from utils import get_seq_list
import hashlib
...
def write_ground_truth(output_file, depth_data, additional_data):
...
def process_directory(mapping_file, input_dir, output_dir):
    ...
    seq_list = get_seq_list(mapping_file, input_path)
    ...
    for sub_path in seq_list:
        depth_dir = os.path.join(input_path, sub_path.replace('outdoor/', 'outdoor_gt/'), 'depth')
        ...
        for filename in os.listdir(depth_dir):
            if filename.endswith('.npz'):
                ...
                if os.path.exists(depth_path) and os.path.exists(image_path) and os.path.exists(event_path):
                    data_list.append([depth_path, image1_index, image_path, event_path, None])
                    candidate_list.append(depth_path)
                else:
                    if not os.path.exists(depth_path):
                        print(f'Missing files: {depth_path}')
    ...
    if generate_vis_list:
        ...
        with open('vis.txt', 'w') as f:
            for vis in vis_list:
                vis = vis.replace(str(input_path)+'/', '').replace('/depth', '')
                f.write(f'{vis}\n')

    for data in tqdm.tqdm(data_list, total=len(data_list), desc="Processing samples"):
        depth_path, image1_index, image_path, event_path, config_path = data
        basename = f'{image1_index}'
        rel_path = os.path.relpath(os.path.dirname(depth_path), input_path).replace('depth', '')
        ...
        image2_index = int(image1_index) + 1
        start_ts = float(image1_index) * (1000.0 / fps)  # Convert to ms
        end_ts = float(image2_index) * (1000.0 / fps)    # Convert to ms

        # read data
        depth_raw_data = np.load(depth_path)['depth'].astype(np.float32)
        valid_mask = depth_raw_data < 100
        height, width = depth_raw_data.shape[:2]

        ground_truth_rel_path = os.path.join(rel_path, f'{basename}.npz')
        ...
        ground_truth_file = ground_truth_dir / rel_path / f'{basename}.npz'
        write_ground_truth(ground_truth_file, depth_raw_data, additional_data)

if __name__ == "__main__":
    # This script is used to generate:
    # 1. sample maps for depth data
    ...
    parser.add_argument('--input_dir', help='Input directory containing .npz files', required=True)
